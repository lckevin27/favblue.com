...
// Authentication
router.post('/loginAPI/login', passport.authenticate('local'), function(req, res, next) {
...
router.get('/loginAPI/validateToken', verifyToken, function(req, res, next) {
  jwt.verify(req.token, 'secretkey', async (err, authData) => {
    if(err) {
      res.send(err);
    } else {
...
router.post('/loginAPI/register', function(req, res, next) {
